...
```bash
sudo mkdir -p /storage/srv/nfs/shared
```

Set ownership:

```bash
sudo chown nobody:nogroup /storage/srv/nfs/shared
sudo chmod 777 /storage/srv/nfs/shared
```
...
```
/storage/srv/nfs/shared 192.168.10.0/24(rw,sync,no_subtree_check)
```
...
```bash
sudo mount 172.16.184.155:/storage/srv/nfs/shared /mnt/nfs
```
...
```
172.16.184.155:/storage/srv/nfs/shared  /mnt/nfs  nfs  defaults  0  0
```
...
```powershell
mount \\192.168.10.10\storage\srv\nfs\shared Z:
```
